extensions [ nw ]
globals [colour counter node2-list existing-connections color-red color-blue team-id-list existing-paths]
turtles-own [ team-number manager? visited? novice?]
...
to go
  ...
  add-fresh-interactions
  add-seasoned-interactions
  add-repeat-interactions
  tick
end
...
to setup-nodes
  ...
  repeat num-teams[
    ...
    crt num-people[
      ...
      set manager? 0
      set novice? 1
      set color-blue color-blue + 1
    ]
    ask turtles with [team-number = counter][fd random 3 ]
    ask one-of turtles with [ team-number = counter]
    [ set manager?  1
      set novice? 0
      set color red
      set color-red color-red + 1
      set color-blue color-blue - 1
    ]
...
to setup-manager-links
  ...
  [
    let node2 turtles with [ team-number = 0 ]
    create-links-with node2
  ]
...
to novice-to-experienced
  let convert f_c * ( count turtles )

  while [ convert > 0]
  [
    let novice-devs one-of other turtles with [novice? = 1 ]
    if novice-devs = nobody
    [ stop ]

    ask one-of turtles with [ novice? = 1 ]
    [
      set novice? 0
      set color red
      set color-red color-red + 1
    ...
    if color-blue - color-red < convert
    [ stop ]
  ]
end

to add-new-devs
  let add-people f_n * ( count turtles)
  foreach team-id-list [
  team-id ->
      let new-dev ( one-of turtles with [team-number = team-id and novice? = 1] )
      ;;print add-people
      if new-dev != nobody
      [
         ask new-dev
         [ hatch  int add-people + 1  ]
         ifelse add-people < 1
         [ set color-blue color-blue + 1 ]
         [ set color-blue color-blue + int add-people]
      ]
...
to add-new-interactions
  ...
  let count-new  ( color-blue * ( color-blue - 1 ) / 2 )
  let new-interactions-edges p_new * count-new

  repeat new-interactions-edges
  [
    let new-dev ( one-of turtles with[ novice? = 1] )
    if new-dev != nobody
    [
      ask new-dev
      [
        let node2 one-of other turtles with [ novice? = 1 and not link-neighbor? self]
        if node2 != nobody
        [add-edge self node2]
      ]
   ]
...
to add-fresh-interactions
  ...
  let count-new  ( color-blue * color-red  )
  ...
  [
    ...
    [
      ask new-dev
      [
        let node2 one-of other turtles with [ novice? = 0 and not link-neighbor? self]
        if node2 != nobody
        [add-edge self node2]
      ]
    ]
...
to add-seasoned-interactions
  ...
  let count-new  ( color-red * ( color-red - 1 ) / 2 )
  let new-interactions-edges p_seasoned * count-new

  repeat new-interactions-edges
  [
    let new-dev one-of turtles with[ novice? = 0]
    if new-dev != nobody
    [
       ask new-dev
       [
         let node2 one-of other turtles with [ novice? = 0]
         if node2 != nobody
         [add-edge self node2]
       ]
    ]
...
to add-repeat-interactions
  ...
  ask links
  [
    ask both-ends with [ color = red ]
    [ set count-link count-link + 1 ]
  ]
  ...
  repeat new-interactions-edges
  [
    ask one-of turtles with[ novice? = 0]
    [
      let node2 one-of other turtles with [ novice? = 0 ]
      if node2 != nobody
      [add-edge self node2]
...
to-report shortest-dist [node1 node2]
  ...
  [ set dist 0]
  if dist > 0
  [ set existing-paths existing-paths + 1 ]

  report dist
end

to-report Degree-of-Separation
  set existing-paths 0
  ...
  [
     ...
     report sum-separation / existing-paths
  ]
  report 0
end
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot Degree-of-Separation"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot Degree-of-Connection"
